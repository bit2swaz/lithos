...
static bool find_latest_sst(const char* dbpath, char* out, size_t out_sz) {
    ...
    while ((ent = readdir(dir)) != NULL) {
        if (!has_sst_suffix(ent->d_name)) continue;
        char* end = NULL;
...
static bool flip_index_block_bit(const char* path) {
    ...
    uint64_t trailer_start = footer.index_handle.offset + footer.index_handle.size; /* type byte */
...
static bool detect_corruption_via_table(const char* sst_path, const Lithos_Options* opt) {
    struct stat st;
...
int main(int argc, char** argv) {
    ...
    if (Status_IsNotFound(s)) {
        printf("Key not found after corruption (treated as failure to detect corruption)\n");
        Status_Free(s);
        Lithos_Close(db);
        return 1;
    }
