...
static void choose_distinct_indices(bool* mask, const bool* exclude, int total, int choose) {
    int picked = 0;
    while (picked < choose) {
        int idx = rand() % total;
...
static int snapshot_test(Lithos_DB* db, const bool* update_mask, const bool* delete_mask) {
    ...
    for (int i = 0; i < KEY_COUNT; i++) {
        if (update_mask[i] || delete_mask[i]) {
            ...
            if (!Status_IsOK(s)) {
                fprintf(stderr, "[Stage 2][snapshot] key=%s status=%s\n", key, Status_ToString(s));
                Status_Free(s);
                warnings++;
                continue;
            }
            if (!(out != NULL && strcmp(out, expected) == 0)) {
                fprintf(stderr, "[Stage 2][snapshot-mismatch] key=%s expected=%s got=%s\n",
                        key, expected, out ? out : "<null>");
                Lithos_Free(out);
                warnings++;
                continue;
            }
    ...
    for (int i = 0; i < KEY_COUNT; i++) {
        if (update_mask[i]) {
            ...
            if (!Status_IsOK(s)) {
                ...
                fprintf(stderr, "[Stage 2][live-update-mismatch] key=%s expected=%s got=%s\n",
...
static void persistence_test(const char* dbpath, Lithos_Options* opt, const bool* update_mask, const bool* delete_mask) {
    Lithos_DB* db = NULL;
    ...
    while (checked < SAMPLE_COUNT) {
        ...
        if (delete_mask[idx]) continue; /* Skip deleted ones. */
