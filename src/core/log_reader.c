 ...
 */
static bool ReadPhysicalRecord(LogReader* reader, Lithos_Slice* result, RecordType* type) {
    while (true) {
        ...
        if (reader->buffer_offset >= reader->buffer_size) {
            ...
            // Read next block
            Lithos_Slice block;
            Status s = SequentialFile_Read(reader->file, kBlockSize, &block, reader->buffer);
