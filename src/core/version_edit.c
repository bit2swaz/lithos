...
/* Tag identifiers (compact, varint encoded). Adding new tags is backward-compatible. */
typedef enum {
    kTagLogNumber = 1,
    kTagPrevLogNumber = 2,
...
void VersionEdit_AddFile(VersionEdit* edit,
                         ...
                         Lithos_Slice smallest,
                         Lithos_Slice largest) {
    if (!EnsureNewCapacity(&edit->new_files, &edit->new_files_cap, edit->new_files_count + 1)) {
        return;
    }
    FileMetaData* meta = FileMetaData_Create(number, file_size, smallest, largest);
    if (meta == NULL) {
        return;
    }
...
void VersionEdit_DeleteFile(VersionEdit* edit, int level, uint64_t number) {
    if (!EnsureDelCapacity(&edit->deleted_files, &edit->deleted_files_cap, edit->deleted_files_count + 1)) {
        return;
    }
...
static void AppendSlice(char** dst, size_t* cap, size_t* len, Lithos_Slice s) {
    ...
    if (*len + s.size > *cap) {
        size_t new_cap = (*cap == 0) ? 64 : (*cap * 2);
        while (*len + s.size > new_cap) new_cap *= 2;
        *dst = realloc(*dst, new_cap);
        *cap = new_cap;
    }
...
Status VersionEdit_EncodeTo(VersionEdit* edit, Lithos_Slice* dst) {
    ...
    if (edit->has_next_file_number) {
        /* Tag 3: next_file_number reserves an ID range so future files do not collide. */
    ...
    for (size_t i = 0; i < edit->deleted_files_count; i++) {
        /* Tag 5: (level, file_number) tombstones remove obsolete files from the version. */
...
static bool GetLengthPrefixedSlice(const char** p, const char* limit, Lithos_Slice* out) {
    uint32_t len = 0;
    if (!GetVarint32(p, limit, &len)) return false;
    if ((size_t)(limit - *p) < len) return false;
...
Status VersionEdit_DecodeFrom(VersionEdit* edit, Lithos_Slice src) {
    ...
    while (p < limit) {
        ...
        switch (tag) {
            case kTagLogNumber: {
                uint64_t v;
                if (!GetVarint64(&p, limit, &v)) return Status_Corruption("bad log number", NULL);
                VersionEdit_SetLogNumber(edit, v);
                break;
            }
            case kTagPrevLogNumber: {
                uint64_t v;
                if (!GetVarint64(&p, limit, &v)) return Status_Corruption("bad prev log", NULL);
                VersionEdit_SetPrevLogNumber(edit, v);
                break;
            }
            case kTagNextFileNumber: {
                uint64_t v;
                if (!GetVarint64(&p, limit, &v)) return Status_Corruption("bad next file", NULL);
            ...
            case kTagDeletedFile: {
                uint32_t level;
                uint64_t num;
                if (!GetVarint32(&p, limit, &level)) return Status_Corruption("bad del level", NULL);
                if (!GetVarint64(&p, limit, &num)) return Status_Corruption("bad del file", NULL);
            ...
            case kTagNewFile: {
                ...
                Lithos_Slice small;
                Lithos_Slice large;
                if (!GetVarint32(&p, limit, &level)) return Status_Corruption("bad new level", NULL);
                if (!GetVarint64(&p, limit, &num)) return Status_Corruption("bad new file num", NULL);
                if (!GetVarint64(&p, limit, &fsize)) return Status_Corruption("bad new file size", NULL);
                if (!GetLengthPrefixedSlice(&p, limit, &small)) return Status_Corruption("bad smallest", NULL);
                if (!GetLengthPrefixedSlice(&p, limit, &large)) return Status_Corruption("bad largest", NULL);
