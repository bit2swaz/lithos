 ...
 * Between restarts, it compresses keys by sharing prefixes with the previous
 * key. The result is a dense block that supports O(log N) seeks with minimal
 * decompression.
 *
...
static void Buffer_Append(Lithos_BlockBuilder *b, const char *data,
  ...
  if (b->buffer_size + size > b->buffer_capacity) {
    ...
    if (new_capacity < b->buffer_size + size) {
      new_capacity =
          b->buffer_size + size; // Or just enough if doubling isn't sufficient
    }
 ...
 * Output: Lithos_BlockBuilder* (ready to accept KV pairs) or NULL on alloc
 * failure Intent: Set up the builder with initial buffer capacities and empty
 * state. The builder starts ready to accept the first key (which becomes a
 * restart).
 */
...
void BlockBuilder_Add(Lithos_BlockBuilder *b, Lithos_Slice key,
  ...
  Buffer_Append(b, key.data + shared,
                non_shared); // Only the differing part of key
