 ...
 */
struct Lithos_Block {
    const char* data;           // Raw block data (owned elsewhere, e.g., cache or file buffer).
    size_t size;                // Total block size including entries and restart array.
    uint32_t restart_offset;    // Byte offset where restart array starts (from block start).
    uint32_t num_restarts;      // How many restart points (each is a uint32 offset).
    bool owned;                 // If true, we free data on destroy (e.g., from heap-alloc'd read).
};
...
typedef struct {
    ...
    Lithos_Comparator cmp; // Comparator for binary search (usually InternalKeyComparator).
...
static uint32_t Block_GetRestartOffset(const Lithos_Block* block, uint32_t index) {
...
static bool BlockIter_ParseEntry(BlockIterator* iter, uint32_t offset) {
    ...
    const char* limit = iter->block->data + iter->block->restart_offset;  // Don't read into restart array
    
    /* Decode the three varint lengths: shared_bytes, non_shared_bytes, value_length */
    uint64_t shared, non_shared, value_len;
    p = GetVarint64Ptr(p, limit, &shared);
    if (!p) goto corruption;  // Varint decode failed (corruption or end of data)
    p = GetVarint64Ptr(p, limit, &non_shared);
    if (!p) goto corruption;
    p = GetVarint64Ptr(p, limit, &value_len);
    if (!p) goto corruption;
    
    /* Sanity checks: shared bytes can't exceed current key, and data must fit */
    if (shared > iter->key_size) goto corruption;  // Can't share more than we have
    if (p + non_shared + value_len > limit) goto corruption;  // Would overflow block
    
    /* Reconstruct full key: keep first 'shared' bytes of prev key + 'non_shared' new bytes */
...
/*
 * BlockIter_SeekToRestartPoint: Binary search restart points to find seek start.
 * ========================================================================
 ...
 */
static uint32_t BlockIter_SeekToRestartPoint(BlockIterator* iter, Lithos_Slice target) {
 ...
 * Intent: Start from the last restart point and scan forward to the final entry.
 *         This requires parsing entries but gives us the true last key.
 */
 ...
 *         entry's data, then parse it. Update restart_index if we cross a boundary.
 */
static void BlockIter_Next(void* state) {
    BlockIterator* iter = (BlockIterator*)state;
    ...
    /* Re-parse the varint lengths to skip: shared_len + non_shared_len + value_len */
    uint64_t shared, non_shared, value_len;
    p = GetVarint64Ptr(p, limit, &shared);
    p = GetVarint64Ptr(p, limit, &non_shared);
    p = GetVarint64Ptr(p, limit, &value_len);
    ...
    if (iter->restart_index + 1 < iter->block->num_restarts) {
        uint32_t next_restart_offset = Block_GetRestartOffset(iter->block, iter->restart_index + 1);
