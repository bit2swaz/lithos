 ...
 *
 * Key Concepts:
 * - Two-level iteration: Index block selects data block, data block scans
 * entries.
...
struct Lithos_Table {
  ...
  const char
      *filter_data; // Owned filter block backing store (heap when present)
};

/* Read a block from disk (data + trailer), then verify CRC and decompress if
 * needed. */
...
static void TwoLevel_InitDataBlock(TwoLevelIterator *iter) {
  ...
  /* Store ownership info; if no cache handle, we own and must destroy on
   * release */
