 ...
 *
 *    This allows:
 *    - Zero-copy comparisons (the SkipList comparator works directly on
 * buffers).
...
static Lithos_Slice *EncodeMemTableEntry(Lithos_Arena *arena, Lithos_Slice key,
  ...
  size_t needed = VarintLength(internal_key_size) + key.size + 8 + // seq + type
 ...
 * We need to extract the internal key portion and compare using
...
bool MemTable_Get(Lithos_MemTable *mem, Lithos_Slice key,
  ...
  // freshest version for this user key (versions are ordered descending by
  // seq).
