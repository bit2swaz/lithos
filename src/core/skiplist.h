 ...
 * It provides O(log N) search, insert, and delete operations with simpler
 * implementation.
 *
 * Architecture:
 * - Multiple levels of linked lists (up to kMaxHeight = 12).
 * - Each node appears in Level 0, and with decreasing probability in higher
 * levels.
 ...
 *
 * - **Contains/Iter (Readers):** Use `memory_order_acquire` when reading next
 * pointers. → Ensures they see the fully initialized node published by the
 * writer.
 *
 ...
 * API Design (Generic):
 * ---------------------
 * Since C lacks templates, we use `void*` keys and a Comparator function
 * pointer. The caller is responsible for key memory management (typically via
 * Arena).
 *
 ...
 *
 * Note: The SkipList itself is allocated on the heap, but all nodes come from
 * the Arena.
 */
 ...
 *
 * Note: This only frees the SkipList struct itself, not the nodes (Arena owns
 * them).
 */
 ...
 * @param key The key to insert (must outlive the SkipList, typically
 * Arena-allocated).
 *
 * **CRITICAL:**
 ...
 *   - Safe to call while another thread is Inserting (as long as Insert is
 * serialized).
 *
 * Complexity: O(log N) average case.
 */
 ...
 *
 * If no such key exists, the iterator is positioned at the end (Iter_Valid =
 * false).
 */
