...
static TableCacheEntry* TableCache_Open(TableCache* cache, FileMetaData* f) {
    if (cache == NULL || f == NULL) return NULL;
...
TableCache* TableCache_Create(const char* dbname, size_t entries) {
    ...
    if (dbname) {
        ...
        c->dbname = malloc(n + 1);
        if (c->dbname) memcpy(c->dbname, dbname, n + 1);
    }
...
static void TableCache_Saver(void* arg, Lithos_Slice ikey, Lithos_Slice val) {
    ...
    if (!ParseInternalKey(ikey, &pik)) return;

    if (pik.type == kTypeDeletion) {
        if (ctx->deleted) *ctx->deleted = true;
        if (ctx->found) *ctx->found = false;
    } else {
        if (ctx->found) *ctx->found = true;
        if (ctx->deleted) *ctx->deleted = false;
        if (ctx->value_out) *ctx->value_out = val;
    }
}
...
Status TableCache_Get(TableCache* cache,
                      ...
                      bool* found,
                      bool* deleted) {
    if (value_out) *value_out = Slice_Empty();
    if (found) *found = false;
    if (deleted) *deleted = false;
    if (cache == NULL || f == NULL) return Status_InvalidArgument("table_cache_get");
    ...
    if (handle == NULL) {
        ...
        handle = Cache_Insert(cache->cache, key, entry, sizeof(TableCacheEntry), TableCache_Deleter);
    }
    ...
    Status s = Table_InternalGet(entry->table, internal_key, &ctx, TableCache_Saver);
...
Lithos_Iterator* TableCache_NewIterator(TableCache* cache,
    ...
    Lithos_Iterator* child = Table_NewIterator(entry->table, options ? options : &cache->options);
    if (child == NULL) {
        Cache_Release(cache->cache, handle);
        return NULL;
    }
