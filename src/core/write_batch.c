...
uint64_t WriteBatch_Sequence(const Lithos_WriteBatch *batch) {
  if (batch == NULL || batch->rep == NULL || batch->size < kWriteBatchHeader)
    return 0;
...
static Status AppendRecord(Lithos_WriteBatch *batch, ValueType type,
...
Status WriteBatch_Append(Lithos_WriteBatch *dst, const Lithos_WriteBatch *src) {
  ...
  dst->size += payload;
  uint32_t total =
      (uint32_t)WriteBatch_Count(dst) + (uint32_t)WriteBatch_Count(src);
  SetCount(dst, total);
  return Status_OK();
}

static Status IterateBytes(const char *data, size_t size,
