 ...
 *    sequence number. Readers see a consistent snapshot without blocking
 * writers.
 *
 * 2. **Delete Operations:**
 ...
 *
 * This searches for the most recent version of the key (highest sequence
 * number).
 *
 * Behavior:
 * ---------
 ...
 * @param snapshot_seq Sequence upper bound (inclusive). Versions newer than
 * this are ignored.
 * @param value_out Output: Pointer to allocated value (if found and
 * kTypeValue).
