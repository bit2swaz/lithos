 ...
 *    Link it into all levels <= its height (atomically at Level 0, sequentially
 * above).
 *
 ...
 *    - This ensures a "happens-before" relationship: if a reader sees a
 * pointer, it also sees all writes that happened before the Release.
 *
 * Visual Example:
 * ---------------
 ...
 *   Expected search time: O(log N) where the base is 1/p (here p=0.5, so base
 * 2).
 *
 * For 1 million elements:
 *   log2(1,000,000) ≈ 20 levels would be ideal.
 *
 * LevelDB uses 12 as a practical tradeoff (supports ~4096 elements
 * efficiently). We follow this convention.
 */
...
typedef struct SkipList_Node {
  ...
  _Atomic(uintptr_t)
      next[]; // Flexible array of atomic pointers (one per level)
} Node;
 ...
 *
 * Note: Flexible array members have zero size, so we allocate space for all
 * height pointers.
 */
...
Lithos_SkipList *SkipList_Create(Lithos_Comparator cmp, Lithos_Arena *arena) {
  ...
  // Allocate the SkipList struct on the heap (NOT from Arena, as it outlives
  // flush cycles)
...
void SkipList_Insert(Lithos_SkipList *list, const void *key) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    //   - prev[i]->next[i] currently points to the node after insertion
    //   position.
    ...
    // This uses Release to ensure x is fully initialized before any reader sees
    // it.
    SetNext(prev[i], i, x);
  }
}
