...
struct Lithos_Arena {
  ...
  /* Statistics */
  _Atomic size_t
      memory_usage; // Total bytes handed out by malloc (atomic for stats).
};
...
char *Arena_AllocateFallbackSlow(Lithos_Arena *arena, size_t bytes) {
  ...
  arena->alloc_ptr =
      aligned + bytes; // Next free byte after the chunk we return
...
char *Arena_AllocateAligned(Lithos_Arena *arena, size_t bytes) {
  ...
  uintptr_t current_mod =
      (uintptr_t)arena->alloc_ptr & (align - 1); // Low bits reveal misalignment
  ...
  size_t needed =
      (align - current_mod) & (align - 1); // If already aligned, this becomes 0
  ...
  if (total_bytes <= arena->alloc_bytes_remaining) {
    ...
    result = Arena_AllocateFallbackSlow(arena,
                                        bytes); // May return unaligned pointer
    if (result == NULL) {
      return NULL;
    }
    ...
    if (current_mod != 0) {
      ...
      result = Arena_Allocate(arena, bytes + needed) +
               needed; // Grab extra padding then skip it
    }
  }
