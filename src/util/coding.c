...
char *EncodeVarint32(char *dst, uint32_t v) {
  ...
  while (v >= B) {
    // Write low 7 bits and set continuation bit (bit 7) to say "more bytes
    // follow".
    *ptr = (uint8_t)(v | B);
  ...
  // Write the final byte (continuation bit clear because this is the last
  // chunk).
  *ptr = (uint8_t)v;
  ptr++;
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (byte < 128) {
      ...
      result |= ((uint32_t)byte)
                << shift; // Place these 7 bits at current shift position
      *value = result;
      return p;
    } else {
      // More bytes coming
      result |= ((uint32_t)(byte & 127))
                << shift; // Mask off flag, keep data bits
