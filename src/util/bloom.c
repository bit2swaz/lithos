 ...
 *         This is the base hash; double hashing generates K hashes from two bases.
 */
...
static void Bloom_CreateFilter(const Lithos_FilterPolicy* policy,
    ...
    for (int i = 0; i < n; i++) {
        ...
        const uint32_t delta = (h >> 17) | (h << 15);  // Second hash for double hashing
