 ...
 * avoid copying strings/keys/values. Think of it as C's take on
 * std::string_view.
 *
...
static inline Lithos_Slice Slice_FromCString(const char *str) {
  return Slice_Create(
      str, strlen(str)); // strlen walks until '\0'; excludes terminator.
}
...
static inline Lithos_Slice Slice_RemovePrefix(Lithos_Slice s,
  ...
  if (Slice_StartsWith(s, prefix)) {
    // Move the data pointer forward by prefix.size bytes; shrink length
    // accordingly.
