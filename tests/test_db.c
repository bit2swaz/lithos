...
static void TestPutGet(void) {
  ...
  Status s =
      Lithos_DB_Put(db, Slice_FromCString("k1"), Slice_FromCString("v1"));
  ASSERT_OK(s);
...
static void TestWriteBatch(void) {
  ...
  ASSERT_OK(
      WriteBatch_Put(batch, Slice_FromCString("a"), Slice_FromCString("1")));
  ASSERT_OK(
      WriteBatch_Put(batch, Slice_FromCString("b"), Slice_FromCString("2")));
...
static void TestRecovery(void) {
  ...
  ASSERT_OK(
      Lithos_DB_Put(db, Slice_FromCString("foo"), Slice_FromCString("bar")));
  Lithos_DB_Close(db);
