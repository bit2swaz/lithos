...
static void Test_MemTable(void) {
  ...
  if (found && Status_IsOK(s)) {
    if (strcmp(value, "value2_newer") == 0) {
      ...
      printf("    " COLOR_RED
             "ERROR: Expected 'value2_newer', got '%s'\n" COLOR_RESET,
             value);
      ASSERT_TRUE(false);
    }
    free(value);
  ...
  if (found) {
    if (!Status_IsOK(s)) {
      printf("    Get('key1') after delete: " COLOR_GREEN
             "NotFound\n" COLOR_RESET);
    } else {
      printf("    " COLOR_RED
             "ERROR: Expected NotFound, but got value: '%s'\n" COLOR_RESET,
             value);
      free(value);
      ASSERT_TRUE(false);
    }
    ...
    printf("    " COLOR_RED
           "ERROR: Should have found deletion marker\n" COLOR_RESET);
    ASSERT_TRUE(false);
  }
...
void Run_MemTableTests(void) { Test_MemTable(); }