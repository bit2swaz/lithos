...
static void Test_DurabilityUtils(void) {
    printf(COLOR_BLUE "[Test] Durability Utils (CRC32C & File I/O)\n" COLOR_RESET);

    // ===== Test 1: CRC32C Checksum =====

    // Known test vector: crc32c("hello") = 0x9a71bb4c
    const char* test_data = "hello";
    uint32_t crc = crc32c_value(test_data, 5);

    printf("  CRC32C('%s') = 0x%08x\n", test_data, crc);

    // Verify against known constant (from LevelDB test suite)
    ASSERT_EQ(crc, 0x9a71bb4c);
    printf(COLOR_GREEN "    Checksum verified\n" COLOR_RESET);

    // Test incremental CRC (extend)
    uint32_t crc_inc = 0;
    ...
    // ===== Test 2: WritableFile Operations =====
    ...
    // ===== Test 3: SequentialFile Operations =====
...
static void Test_LogFragmentation(void) {
    ...
    // Test 1: Small record (should be FULL type)
    const char* small_data = "tiny";
    Lithos_Slice small_slice = Slice_FromCString(small_data);
    s = LogWriter_AddRecord(writer, small_slice);
    ASSERT_OK(s);
    printf("  Wrote small record: '%s' (%zu bytes)\n", small_data, small_slice.size);

    // Test 2: Large record (70KB - should span ~3 blocks)
    // Block size = 32KB, header = 7B, so max payload per block = 32761B
    // 70KB needs: FIRST (32761) + MIDDLE (32761) + LAST (4478)
    ...
    ASSERT_OK(s);
    printf("  Wrote large record: %zu bytes " COLOR_GREEN "(fragmented)" COLOR_RESET "\n",
           large_size);

    // Test 3: Another small record after fragmentation
    ...
    LogReader* reader = LogReader_Create(rfile, true);  // Enable checksum verification
    ...
    printf("  Read large record: %zu bytes " COLOR_GREEN "(reassembled, verified)\n" COLOR_RESET,
           record.size);

    // Read Record 3: Second small record
    ASSERT_TRUE(LogReader_ReadRecord(reader, &record, &scratch));
